  ...
  void Update() {
    if (_material != null) {
      ...
      if (_intObj is InteractionButton && (_intObj as InteractionButton).isPressed) {
		//Qui gestisco l'interazione del bottone da parte del Player.
		//Quindi il rilascio di oggetti, in questo caso.
				if (this.GetComponent<ItemControllerUI> () != null) {
					if (PlayerVRControllerLeap.getSingleton () != null) {
						PlayerVRControllerLeap.getSingleton ().leaveItem (this.GetComponent<ItemControllerUI>().getTypeItem());
					}
				}
