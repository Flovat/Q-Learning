    ...
    public void BeginGrasp(List<InteractionController> controllers) {
      ...
      if (_graspingControllers.Count == controllers.Count) {
        ...
        OnGraspBegin();
				...
				if (this.GetComponent<ItemScript> () != null) {
					PlayerVRControllerLeap.getSingleton ().ShowCanvasItemPlayer ();
					this.GetComponent<ItemScript> ().setPicked (true);	
				}
      }
    }
    ...
    public void EndGrasp(List<InteractionController> controllers) {
      ...
      if (_graspingControllers.Count == 0) {
        ...
        if (_justGrasped) _justGrasped = false;
			...
			Debug.Log ("Oggetto Lasciato");
			if (this.GetComponent<ItemScript> () != null) {
				PlayerVRControllerLeap.getSingleton ().HideCanvasItemPlayer ();
				this.GetComponent<ItemScript> ().setPicked (false);	
			}
    }
